...
# Training the model
if len(sys.argv) <= 1:
    print('Provide argument for the model type\nl -> Linear Regression\nr -> Ridge Regression\nlas -> Lasso Regression')
    sys.exit()
...
print(len(y_test))
print(len(y_pred))
#accuracy = 'accuracy = %.3f' % metrics.accuracy_score(y_test, y_pred)
#precision = 'Precision = %.3f' % metrics.precision_score(y_test, y_pred)
#recall = 'recall = %.3f' % metrics.recall_score(y_test, y_pred)
#f1 = 'f1 = %.3f' % metrics.f1_score(y_test, y_pred)
print(scorestr)
print(errstr)
#print(accuracy)
#print(precision)
